...
SHELL := /bin/bash
NAME := logrus-stackdriver-formatter
BINARY_NAME := logrus-stackdriver-formatter
BUILD_TARGET = build
MAIN_SRC_FILE=./pkg/stackdriver
...
BUILDFLAGS :=  -ldflags \
  " -X $(ROOT_PACKAGE)/pkg/cmd/version.Version=$(VERSION)\
		-X github.com/jenkins-x-plugins/jx-pipeline/pkg/cmd/version.Version=$(VERSION)\
